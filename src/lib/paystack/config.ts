// Note: Do NOT import Paystack SDK here.
// This module is shared by client and server for pricing utilities only.
// Server-only Paystack initialization lives in `src/lib/paystack/server.ts`.
...
export const SIZ_TOKEN_PRICING = {
  ...
  PROCESSING_FEE_PERCENTAGE: 0.015, // 1.5% Paystack processing fee (lower than Stripe)
  PROCESSING_FEE_FIXED: 0.00, // No fixed fee for Paystack
} as const;

// Paystack Product Configuration
export const PAYSTACK_PRODUCT_CONFIG = {
  PRODUCT_NAME: 'SIZ Token',
  PRODUCT_DESCRIPTION: 'SIZ Token - Learn. Earn. Invest. Grow.',
  DEFAULT_CURRENCY: 'USD', // Default to USD for international support
  SUPPORTED_CURRENCIES: ['USD', 'KES', 'NGN'] as const,
  PAYMENT_METHOD_TYPES: ['card', 'bank', 'ussd', 'qr', 'mobile_money'] as const,
...
export function convertUSDToCurrency(usdAmount: number, targetCurrency: string): number {
  switch (targetCurrency.toUpperCase()) {
    case 'NGN':
      return Math.round(usdAmount * CURRENCY_RATES.USD_TO_NGN);
    case 'KES':
      return Math.round(usdAmount * CURRENCY_RATES.USD_TO_KES);
    case 'USD':
      return usdAmount;
    default:
      return usdAmount; // Default to USD if currency not supported
  }
}

// Convert any currency to USD
export function convertCurrencyToUSD(amount: number, fromCurrency: string): number {
  switch (fromCurrency.toUpperCase()) {
    case 'NGN':
      return Number((amount * CURRENCY_RATES.NGN_TO_USD).toFixed(2));
    case 'KES':
      return Number((amount * CURRENCY_RATES.KES_TO_USD).toFixed(2));
    case 'USD':
      return amount;
    default:
      return amount; // Default to USD if currency not supported
  }
}
...
export function convertUSDToNGN(usdAmount: number): number {
  return convertUSDToCurrency(usdAmount, 'NGN');
}

export function convertNGNToUSD(ngnAmount: number): number {
  return convertCurrencyToUSD(ngnAmount, 'NGN');
}
