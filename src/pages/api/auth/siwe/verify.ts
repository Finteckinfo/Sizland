...
export default async function handler(
  ...
  try {
    ...
    const expectedDomain = process.env.NEXTAUTH_URL?.replace(/^https?:\/\//, '') || 'localhost:3000';
    if (siweMessage.domain !== expectedDomain && process.env.NODE_ENV === 'production') {
      console.warn('[Security] Domain mismatch:', { expected: expectedDomain, received: siweMessage.domain });
      return res.status(401).json({ 
        error: 'Domain mismatch',
        message: 'Invalid authentication domain'
      });
    }

    // Validate message issued time is not too old (prevent replay with old signatures)
    if (siweMessage.issuedAt) {
      const issuedTime = new Date(siweMessage.issuedAt).getTime();
      const now = Date.now();
      ...
      if (issuedTime > now + 60000) { // Allow 1 minute clock skew
        return res.status(401).json({ 
