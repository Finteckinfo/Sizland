...
export function CatalogExplorer({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={`flex w-full flex-col gap-3 ${embedded ? '' : 'mx-auto max-w-[1600px] px-3 pb-8 pt-4 sm:px-4 lg:px-6'}`}>
        ...
        <section className={`relative min-h-[320px] flex-1 overflow-hidden rounded-2xl border ${panelClass}`}>
          {stageMode === 'MAP' ? (
            ...
            <div className="flex h-full min-h-[420px] flex-col items-center justify-center p-6">
              <p className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Commodity media stage</p>
            </div>
          )}
        </section>
        ...
        {selected && (
          ...
          >
            <div className="sticky top-0 flex items-center justify-between border-b border-border/50 bg-inherit px-4 py-3">
              ...
              <button type="button" aria-label="Close detail" onClick={() => setSelectedId(null)} className="rounded-full p-1">
                <X className="h-5 w-5" />
              </button>
            </div>
